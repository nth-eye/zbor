 ...
 * 
 */
struct codec : impl::interface<codec> {
    friend impl::interface<codec>;
    constexpr codec() = delete;
    constexpr codec(std::span<byte> buf) : buf{buf.data()}, max{buf.size()} {}
private:
    byte* buf;
    size_t max;
};
...
/**
 * @brief Wrapper for codec wit internal storage.
 * 
 * @tparam N Buffer size in bytes
 */
template<size_t N>
struct buffer : impl::interface<buffer<N>> {
    friend impl::interface<buffer<N>>;
    constexpr operator codec() { return buf; }
private:
...
//     constexpr codec() = delete;
//     constexpr codec(std::span<byte> buf) : buf{buf.data()}, max{buf.size()} {}
//     constexpr codec(codec&&) = default;
//     constexpr codec(const codec&) = default;
//     constexpr codec& operator=(codec&&) = default;
//     constexpr codec& operator=(const codec&) = default;
...
//     constexpr err encode(auto... args)
//     {
//         err e = err_ok;
//         ((e = encode(args)) || ...);
//         return e;
...
// constexpr err codec::encode_base(byte start, uint64_t val, size_t ai_len, size_t add_len)
// {
//     if (idx + ai_len + add_len + 1 > max)
//         return err_no_memory;
...
// constexpr err codec::encode_head(mt_t mt, uint64_t val, size_t add_len)
// {
//     byte ai;
...
// constexpr err codec::encode_bytes(mt_t mt, const void* data, size_t len)
// {
//     err e = encode_head(mt, len, len);
//     if (e == err_ok && data && len) {
//         std::copy_n(static_cast<const byte*>(data), len, buf + idx);
...
// constexpr err codec::encode_float(float val)
// {
//     if (val != val)
//         return encode_nan();
